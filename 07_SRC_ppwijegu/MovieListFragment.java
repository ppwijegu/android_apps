...
//
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        DatabaseReference childRef =
                FirebaseDatabase.getInstance().getReference().child("moviedata").getRef();
        mAdapter = new MyFirebaseRecylerAdapter(Movie.class,
                R.layout.movie_card_layout, MyFirebaseRecylerAdapter.MovieViewHolder.class,
                childRef, getContext(),this);

        moviesRV.setAdapter(mAdapter);

        // Create adapter passing in the sample user data
        if (movieData.getSize() == 0) {
            movieData.setAdapter(mAdapter);
            movieData.setContext(getActivity());//getApplicationContext()-activity is used
    ...
    public void copy(int position){
        ...
        HashMap movie = (HashMap) movieData.getItem(position).clone();
        movie.put("id",movie.get("id")+"_new");
